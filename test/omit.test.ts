...
test("utils: object key omit", async (t) => {
    ...
    await t.test("omit default keys parameter test", async () => {
        ...
        assert.equal(
            "table" in <typeof testingObject> utils.omit(testingObject),
            false,
        );
    });
    ...
    await t.test("omit empty keys parameter test", async () => {
        ...
        assert.equal(
            "session" in <typeof testingObject> utils.omit(testingObject, []),
            true,
        );
    });

    await t.test(
        "omit custom object property passed in keys parameter",
        async () => {
            /** omit custom property */
            assert.equal(
                "fullname"
                    in <typeof testingObject> utils.omit(testingObject, ["fullname"]),
                false,
            );
        },
    );
    ...
    await t.test("omit multiple custom object properties", async () => {
        ...
        const omitKeys = <typeof testingObject> (
            utils.omit(testingObject, ["hobby", "session"])
        );
